#!/bin/bash
# =============================================================================
# Configure Suricata IDS on VM1 (Stockholm GW) and VM4 (London GW)
# =============================================================================
# Run from the project directory (where Vagrantfile lives):
#   bash services/ids/setup-suricata.sh
#
# Prerequisites: VM1 and VM4 provisioned (suricata package already installed)
#
...
# ─────────────────────────────────────────────────────────
echo ">>> Configuring Suricata on VM1 (Stockholm Gateway)..."

vagrant ssh vm1-gw -- sudo bash -s << 'SURI_VM1_EOF'
set -euo pipefail
...
# Patch HOME_NET to cover all Stockholm + London subnets
...
# Set the primary capture interface to enp0s8 (VLAN 10 — internal traffic)
# Use af-packet for live capture
...
# ── 2. Update rulesets (ET Open) ──
echo ">>> Updating Suricata rulesets (this may take a moment)..."
...
# =============================================================================
# ACME Custom Suricata Rules — Stockholm Gateway
# =============================================================================
...
# ── IPsec Probe (non-established IKE traffic) ──
...
# ── Unauthorized Access to CA Server (air-gapped VM3) ──
alert ip any any -> 10.0.1.3 any (msg:"ACME: Unauthorized traffic to CA server (VM3)"; threshold:type both,track by_src,count 1,seconds 60; sid:9000006; rev:1;)
RULES_EOF

# Include custom rules file in suricata config if not already present
...
echo ">>> Suricata active on VM1 (enp0s8)."
...
echo ""
echo "====================================================="
echo " Suricata IDS setup complete on VM1 and VM4!"
echo ""
echo " Monitoring interfaces:"
echo "   VM1: enp0s8 (Stockholm VLAN 10)"
...
echo "   9000006 — Unauthorized traffic to CA (VM1 only)"
echo ""
echo " Log files:"
